...
class TestPrepareForOnSky(
    ...
    async def test_configure(self):
        async with self.make_script():
            ...
            assert self.script.target_az is None
            assert self.script.target_el is None
            assert self.script.target_rot is None
    ...
    async def test_configure_invalid_target_el(self):
        async with self.make_script():
            with self.assertRaises(salobj.ExpectedError):
                await self.configure_script(
                    target_el=MTCS.tel_operate_mirror_covers_el - 1.0,
                )

            with self.assertRaises(salobj.ExpectedError):
                await self.configure_script(
                    target_el=MTCS.tel_max_el + 1.0,
                )
