...
class BaseLsstCamCheckout(salobj.BaseScript):
    """Common implementation for LSSTCam OODS ingestion checkouts.

    Concrete subclasses select the number and duration of DARK exposures.
    This base class verifies that LSSTCam is enabled, logs the current
    instrument configuration, takes the requested DARK frames, and checks
    that they were successfully ingested by MTOODS and WFOODS.
    ...
    descr : `str`
        Description of the concrete SAL script.
    ...
    **Checkpoints**
    ...
    wavefront-sensor ingestions from WFOODS. After science ingestion completes,
    the script waits up to ``self.guider_ingestion_grace_period`` seconds for
    eight guider-sensor ingestions. Guider ingestion is reported but is not
    required by the checkout.

    DARK frames exercise image acquisition and OODS ingestion, including WFS
    ...
    def get_schema(cls):
        ...
        schema_yaml = """
            ...
            $id: https://github.com/lsst-ts/ts_maintel_standardscripts/base_lsstcam_checkout.yaml
            title: BaseLsstCamCheckout v1
            description: Common configuration for LSSTCam OODS ingestion checkouts.
            type: object
            properties:
              program:
    ...
    async def configure_camera(self) -> None:
        """Initialize LSSTCam if it is not already initialized."""
        if self.lsstcam is None:
            self.log.debug("Creating LSSTCam instance.")
            ...
            self.log.debug("LSSTCam already initialized.")

    async def configure_wfoods(self) -> None:
        """Initialize the WFOODS remote if it is not already initialized."""
        if self.wfoods is None:
            self.log.debug("Creating WFOODS remote.")
            ...
            self.log.debug("WFOODS remote already initialized.")

    async def configure(self, config):
        """Configure the script, LSSTCam object, and WFOODS remote.
    ...
    def set_metadata(self, metadata):
        ...
        dark_duration = (
            ...
            + self.guider_ingestion_grace_period
    ...
    async def log_setup_info(self):
        ...
        Failures are logged as warnings and do not abort the check.
    ...
    async def ingested_image(
        ...
        and WFS ingestions are received. Once science ingestion is complete,
        guider ingestion is observed for up to
        ``self.guider_ingestion_grace_period`` additional seconds. Missing
        guider ingestions are reported but do not fail validation.
    ...
    async def _collect_ingestion_events(
        ...
        Notes
        -----
        MTOODS collection requires science sensors and then continues for a
        configured interval to observe guider ingestion. WFOODS collection
        requires WFS sensors and has no additional ingestion phase.
        ...
        if required_expected_count == 0:
            return ingestion_events, observed_obsid

        async def collect_events(target_pairs, target_count, event_timeout):
            nonlocal observed_obsid
            while len(target_pairs) < target_count:
                # Wait for the next ingestion event
                try:
                    ingest_event = await topic.next(flush=False, timeout=event_timeout)
        ...
        if (
            ...
            remaining = guider_expected_count - len(guider_pairs)
            self.log.info(
                f"Waiting up to {self.guider_ingestion_grace_period} seconds for "
                f"the remaining {remaining} of {guider_expected_count} guider "
                f"sensor ingestions for obsid {observed_obsid}."
            )
            try:
                await asyncio.wait_for(
                    collect_events(
                        guider_pairs,
                        guider_expected_count,
                        self.guider_ingestion_grace_period,
                    ),
                    timeout=self.guider_ingestion_grace_period,
                )
            except asyncio.TimeoutError:
                pass
    ...
    def _validate_ingestion(
        ...
        if guider_count >= self.expected_dark_ingest_guider:
            self.log.info(
                f"Observed {guider_count}/{self.expected_dark_ingest_guider} "
                f"guider sensor ingestions for obsid {observed_obsid}."
            )
        elif guider_count == 0:
            self.log.warning(
                ...
                f"requested initialization of all "
                f"{self.expected_dark_ingest_guider} default guider ROIs, but "
                f"the camera may not be configured to produce guider data."
            )
        else:
            self.log.warning(
                f"Incomplete {image_label} guider-sensor ingestion for obsid "
                ...
                f"{self.guider_ingestion_grace_period} seconds after science "
                f"ingestion completed for obsid {observed_obsid}. "
                f"LSSTCam requested all {self.expected_dark_ingest_guider} "
                f"default guider ROIs. Check the camera guider configuration "
                f"and MTOODS ingestion."
            )
