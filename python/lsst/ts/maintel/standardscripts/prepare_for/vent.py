...
from lsst.ts.observatory.control.constants import mtcs_constants
...
VENT_PARAMS = mtcs_constants.SimonyiVentParameters()
...
class PrepareForVent(salobj.BaseScript):
    ...
    pointed to the dome's azimuth at ``VENT_PARAMS.tel_vent_elevation`` at
    the start of the script, and again once the sun drops to or below
    ``VENT_PARAMS.sun_elevation_horizon`` (see below) -- it is not moved at
    any other point.
    ...
    when the sun drops to or below ``VENT_PARAMS.sun_elevation_horizon``,
    since the sun-avoidance constraint no longer applies at that point (the
    telescope is repositioned to match at that same moment). Until the sun
    reaches ``VENT_PARAMS.sun_elevation_high``, the shutter and louvers are
    left untouched entirely, and the dome is only ever nudged the minimum
    amount needed to stay clear of the sun (see `compute_sun_safe_azimuth`);
    this also continues to be the only dome movement between
    ``VENT_PARAMS.sun_elevation_high`` and
    ``VENT_PARAMS.sun_elevation_horizon`` once the shutter/louvers do start
    opening. The louvers/shutter are opened progressively as the sun sets,
    staying compliant with the EAS dome sun-avoidance model. By the time the
    sun reaches ``VENT_PARAMS.sun_elevation_stop`` the dome shutter and all
    louvers are fully opened and the script ends. If the temperature
    condition is never met, the dome is repositioned for the wind one last
    time and the shutter/louvers are opened directly once the sun reaches
    ``VENT_PARAMS.sun_elevation_stop``, without the progressive adjustments.
    ...
    **Checkpoints**
    ...
    - "Sun at ... deg elevation, waiting ...s for sun to drop below ... deg
      (venting start limit)": once per loop iteration while waiting for the
      sun to reach ``VENT_PARAMS.sun_elevation_high`` (see
      `wait_for_sun_elevation_high`); the dome stays at its away-from-sun
      position during this wait, only nudged for sun-avoidance, and the
      shutter/louvers are not touched.
    - "Positioning dome for the current wind direction": before the one-time
      wind-facing dome slew, once the sun has reached
      ``VENT_PARAMS.sun_elevation_high``.
    - "Venting: sun at ... deg elevation, ... deg azimuth; dome at ... deg
      azimuth...": once per loop iteration between
      ``VENT_PARAMS.sun_elevation_high`` and
      ``VENT_PARAMS.sun_elevation_horizon`` (see `vent_while_sun_sets`).
    - "Sun elevation below ... deg (sun horizon limit), positioning dome and
      telescope to point into wind ...": once per loop iteration at or below
      ``VENT_PARAMS.sun_elevation_horizon`` (see `vent_while_sun_sets`).
    - "Sun reached ... deg (venting stop limit) before the temperature
      condition was met; opening dome shutters and louvers to 100 percent":
      if venting never started because the temperature condition was never
      met.
    """
    ...
    def __init__(self, index):
        ...
        self.loop_wait_time = VENT_PARAMS.loop_wait_time
    ...
    def set_metadata(self, metadata):
        metadata.duration = self.estimate_time_until_sun_elevation(
            VENT_PARAMS.sun_elevation_stop
        )
    ...
    async def wait_for_vent_condition(self):
        ...
        `VentCondition`
            ...
            ``VENT_PARAMS.sun_elevation_stop`` before that happens.
        """
        outside_temp = (
        ...
        while sun_el > VENT_PARAMS.sun_elevation_stop and (
            outside_temp is None
            or indoor_temp is None
            or outside_temp - indoor_temp
            >= VENT_PARAMS.temperature_differential_threshold
        ):
            self.log.info(
        ...
        if condition_met:
            ...
            self.log.warning(
                f"Sun reached {VENT_PARAMS.sun_elevation_stop} deg "
                "(venting stop limit) before the temperature condition was "
                "met."
            )
    ...
    async def reposition_dome_for_wind(self, sun_az, clamp_to_sun_avoidance_range):
        ...
        ``VENT_PARAMS.sun_elevation_horizon`` (with
        ...
        clamp_to_sun_avoidance_range : `bool`
            ...
            ``[VENT_PARAMS.dome_min_az, VENT_PARAMS.dome_max_az]``.
        ...
        if clamp_to_sun_avoidance_range:
            dome_az = min(
                max(dome_az, VENT_PARAMS.dome_min_az), VENT_PARAMS.dome_max_az
            )
    ...
    def compute_sun_safe_azimuth(dome_az, sun_az):
        ...
        ``VENT_PARAMS.louver_sun_avoidance_angle`` away from the sun.
        ...
        -------
        `float`
            A dome azimuth at least ``VENT_PARAMS.louver_sun_avoidance_angle``
            from the sun, on the same side of the sun as ``dome_az``.
        """
        offset = angle_diff(dome_az, sun_az).deg
        if abs(offset) >= VENT_PARAMS.louver_sun_avoidance_angle:
            return dome_az
        edge = (
            VENT_PARAMS.louver_sun_avoidance_angle
            if offset >= 0.0
            else -VENT_PARAMS.louver_sun_avoidance_angle
        )
    ...
    def compute_louver_positions(dome_az, sun_az):
        ...
        Louvers more than ``VENT_PARAMS.louver_sun_avoidance_angle`` from
        the sun are opened to 100%; louvers within that angle are capped at
        ``VENT_PARAMS.louver_sun_exposed_percent``, matching the ts_eas
        DomeModel defaults so the EAS sun-avoidance logic never needs to
        ...
        return {
            MTDome.Louver(i + 1).name: (
                ...
                > VENT_PARAMS.louver_sun_avoidance_angle
                else VENT_PARAMS.louver_sun_exposed_percent
            )
    ...
    async def open_dome_shutter_if_needed(self):
        ...
        if self._shutter_opened:
            self.log.info("Dome Shutter already opened, nothing to do.")
            return
    ...
    async def open_dome_louvers_if_needed(self, position):
        ...
        if position == self._commanded_louver_position:
            self.log.info("Dome louvers already in position, nothing to do.")
            return
    ...
    async def wait_for_sun_elevation_high(self):
        """Wait until the sun descends to ``VENT_PARAMS.sun_elevation_high``.
        ...
        while sun_el > VENT_PARAMS.sun_elevation_high:
            wait_time = self.estimate_time_until_sun_elevation(
                VENT_PARAMS.sun_elevation_high
            )
            await self.checkpoint(
                f"Sun at {sun_el:.2f} deg elevation, waiting {wait_time:.0f}s "
                f"for sun to drop below {VENT_PARAMS.sun_elevation_high} deg "
                "(venting start limit)."
            )
            ...
            safe_az = min(
                max(
                    self.compute_sun_safe_azimuth(dome_az, sun_az),
    ...
    async def vent_while_sun_sets(self):
        ...
        the sun gets lower, until the sun reaches
        ``VENT_PARAMS.sun_elevation_stop``. By the time this loop exits, the
        shutter and louvers are already fully open (they reach that state
        progressively, once the sun is at or below
        ``VENT_PARAMS.sun_elevation_horizon``; see below).

        The dome is repositioned for the wind (see `reposition_dome_for_wind`)
        once, right after waiting for the sun to reach
        ``VENT_PARAMS.sun_elevation_high`` (see `wait_for_sun_elevation_high`,
        which leaves the dome at whatever azimuth `run` last pointed it to --
        away from the sun -- only nudging it as needed for sun-avoidance
        while it waits, and leaves the shutter/louvers untouched entirely),
        and once more the first time the sun is found at or below
        ``VENT_PARAMS.sun_elevation_horizon`` -- at which point the
        telescope is also repositioned to match, since the sun-avoidance
        constraint no longer applies. In between, and on every other loop
        iteration while the sun is still above
        ``VENT_PARAMS.sun_elevation_horizon``, the dome is only nudged the
        minimum amount needed to stay clear of the sun -- it is never
        otherwise moved for the wind. The aperture shutter and louvers are
        commanded open every iteration from here on -- the louvers either
        capped for sun avoidance (see `compute_louver_positions`) while the
        sun is above ``VENT_PARAMS.sun_elevation_horizon`` or fully open
        once it is not -- but `open_dome_shutter_if_needed`/
        `open_dome_louvers_if_needed` only actually send a command when the
        desired state has changed since the last one sent.
        """
        ...
        while sun_el > VENT_PARAMS.sun_elevation_stop:
            if sun_el > VENT_PARAMS.sun_elevation_horizon:
                dome_az = await self.get_dome_azimuth()
                safe_az = min(
                    max(
                        self.compute_sun_safe_azimuth(dome_az, sun_az),
                ...
                await self.checkpoint(
                    ...
                    "azimuth. This is the closest allowable position to the "
                    "wind while sun is above horizon limit of "
                    f"{VENT_PARAMS.sun_elevation_horizon} deg."
                )
                ...
                if not repositioned_for_wind_after_sunset:
                    ...
                    self.log.info(
                        ...
                        f"{VENT_PARAMS.tel_vent_elevation} deg elevation."
                    )
                ...
                await self.checkpoint(
                    f"Sun elevation below {VENT_PARAMS.sun_elevation_horizon} deg "
                    "(sun horizon limit), positioning dome and telescope "
                    f"to point into wind ({dome_az:.1f} deg)."
                )
    ...
    async def run(self):
        ...
        dome_target_az = min(
            max((sun_az + 180.0) % 360.0, VENT_PARAMS.dome_min_az),
            VENT_PARAMS.dome_max_az,
        )
        ...
        await self.checkpoint(
            ...
            f"outside={self._format_temperature(outside_temp)}, "
            f"indoor={self._format_temperature(indoor_temp)}, "
            f"limit={VENT_PARAMS.temperature_differential_threshold} C."
        )
        ...
        if vent_condition == VentCondition.TEMPERATURE_CONDITION_MET:
            ...
            await self.checkpoint(
                f"Sun reached {VENT_PARAMS.sun_elevation_stop} deg "
                "(venting stop limit) before the temperature condition was "
                "met; opening dome shutters and louvers to 100 percent."
            )
            sun_az, _ = self.mtcs.get_sun_azel()
